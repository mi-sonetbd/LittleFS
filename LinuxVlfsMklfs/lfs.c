...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size) {
    ...
    // Track block and offset
    lfs_block_t block = file->block;  // Current block being read
    lfs_off_t offset = file->pos;     // Current offset in the file

    LFS_DEBUG("Reading from block %"PRIu32" at offset %"PRIu32, block, offset);

    // Call the actual file read function
