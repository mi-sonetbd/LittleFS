...
(() => {
  ...
  function createAndShowDialog(title, body, buttons) {
    ...
    // Adjust dialog container to appear centered on the current viewport
    const dialogContainer = $('div', ['z-50', 'fixed', 'left-1/2', 'top-1/2', 'transform', '-translate-x-1/2', '-translate-y-1/2', 'pointer-events-none', 'max-h-screen', 'overflow-auto']);
    
    // Limit dialog height and allow scroll if content exceeds viewport
    const dialog = $('div', ['max-w-screen-md', 'max-h-screen', 'bg-white', 'p-4', 'border', 'rounded-lg', 'shadow-lg', 'pointer-events-auto', 'overflow-y-auto']);
    
    dialog.style.maxHeight = '95vh'; // Max height at 90% of viewport height
    dialog.style.width = '95vw'; // Set width to 90% of viewport width if needed
    ...
    // Function to close the dialog and re-enable background scrolling
    function close() {
  ...
  // Updated Function for Raw Data Handling
function convertBinaryToRaw(buffer) {
  const decoder = new TextDecoder('utf-8', { fatal: false });
  let decodedString;

  try {
    decodedString = decoder.decode(buffer); // Attempt to decode as text
  } catch (e) {
    return convertBinaryToBase64(buffer); // Fallback to Base64 if not decodable as text
  }

  // Function to apply the color to specific characters in JSON
function formatJsonString(jsonString) {
  return jsonString
    .replace(/({|}|"|,)/g, '<span style="color: #505050;">$1</span>') ; // Red commas
}


  // Apply JSON formatting if it's a JSON-like string
  try {
    const jsonObj = JSON.parse(decodedString);
    return formatJsonString(JSON.stringify(jsonObj, null, 2));  // Pretty print the JSON string with styling
  } catch (e) {
    // If it's not JSON, return the plain decoded string
  ...
  function displayDecodedData(logData, hexData) {
    const decodedDataDiv = document.createElement('div');
    decodedDataDiv.className = 'my-4';

    const hexBytes = hexData.trim().split(' ');

    // Function to format hex values with colors
    function formatHexWithColors(bytes) {
      return bytes.map((byte, index) => {
        const color = (index % 2 === 0) ? '#00BFFF' : '#FFA500'; // Alternate colors for hex values
        return `<span style="color: ${color};">${byte}</span>`;
      }).join(' ');
    }

    decodedDataDiv.innerHTML = `
      <div class="tracking-widest uppercase text-xs">Decoded Log Data (with Hex)</div>
      <pre class="whitespace-pre-wrap code-block">
      <table class="table-auto w-full text-left">
        <tr>
          <th class="p-2">Decoded Data</th>
          <th class="p-2">Hex Values</th>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Date:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.date}/${logData.month}/20${logData.year}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(0, 3))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Time:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.hours}:${logData.minutes}:${logData.seconds}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(3, 6))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Latitude:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.lat.toFixed(6)}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(6, 10))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Longitude:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.lng.toFixed(6)}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(10, 14))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Speed:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.speed.toFixed(2)} km/h</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(14, 18))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Course:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.course.toFixed(2)} degrees</span></td>
          <td class="p-2 pl-4">${formatHexWithColors(hexBytes.slice(18, 22))}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Satellites:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.sat}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors([hexBytes[22]])}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Battery:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.bat}%</span></td>
          <td class="p-2 pl-4">${formatHexWithColors([hexBytes[23]])}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">SOS:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.sos ? 'Yes' : 'No'}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors([hexBytes[24]])}</td>
        </tr>
        <tr>
          <td class="p-2 pl-4"><span style="font-weight: bold; color: #00BFFF;">Charger Connected:</span> <span style="font-size: 1.1em; color: #FFA500;">${logData.crg ? 'Yes' : 'No'}</span></td>
          <td class="p-2 pl-4">${formatHexWithColors([hexBytes[25]])}</td>
        </tr>
      </table>
      </pre>
    `;
...
rawDataDiv.innerHTML = `
  ...
  <pre class="whitespace-pre-wrap code-block">${rawView}</pre>
`;
infoBox.appendChild(rawDataDiv);
    
    function formatHex(hexString) {
  // Split the hex string into groups of two characters (one byte per group)
  return hexString.match(/.{1,2}/g).join(' ');
}


function formatHexWithColors(hexString) {
    // Ensure each byte is formatted as a 2-character string (hexadecimal)
    const cleanHexString = hexString.replace(/\s+/g, '');  // Remove any spaces or line breaks if present
    const bytes = cleanHexString.match(/.{1,2}/g);  // Split into pairs of two characters

    if (!bytes) return ''; // In case hexString is empty or invalid

    return bytes.map((byte, index) => {
        const color = (index % 2 === 0) ? '#00BFFF' : '#FFA500'; // Even bytes green, odd bytes red
        return `<span style="color: ${color};">${byte.padStart(2, '0')}</span>`;  // Ensure two characters per byte
    }).join(' ');
}
...
hexDataDiv.innerHTML = `
  ...
  <pre class="whitespace-pre-wrap code-block"><code>${formatHexWithColors(hexView)}</code></pre>
`;
infoBox.appendChild(hexDataDiv);


    // Display Decoded Log Data
    const decodedDataDiv = displayDecodedData(logData, hexView);
